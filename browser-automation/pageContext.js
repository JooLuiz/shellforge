...
async function pollPageCondition(browser, page, evaluateFn, args, step, logInfo) {
  ...
  throw new Error(
    `[Browser Automation] Timed out after ${timeout}ms waiting for page condition`
  );
}
...
async function getTargetFrame(browser, page, step, logInfo) {
  if (!step.iframe) {
    return page;
  }

  page = await waitForSelectorSafe(browser, page, step.iframe, step, logInfo);

  const iframeElement = await page.$(step.iframe);

  if (!iframeElement) {
    throw new Error(`[Browser Automation] Iframe element not found: ${step.iframe}`);
  }

  const frame = await iframeElement.contentFrame();

  if (!frame) {
    throw new Error(`[Browser Automation] Could not get contentFrame for ${step.iframe}`);
  }
...
function getAllChildFrames(frame) {
  ...
  let allFrames = [...children];

  for (const child of children) {
    allFrames = allFrames.concat(getAllChildFrames(child));
  }
...
async function findSelectorInFrameTree(frame, selector) {
  try {
    ...
    // Frame might not be ready, skip it
  }
...
async function pollFrameForSelector(browser, page, selector, step, logInfo) {
  ...
  throw new Error(
    `[Browser Automation] Timed out after ${timeout}ms waiting for ${selector} in iframe ${step.iframe}`
  );
}
...
async function clickSelectorSafe(browser, page, selector, step, logInfo) {
  ...
  if (!element) {
    throw new Error(`[Browser Automation] Element not found for click: ${selector}`);
  }
